...
function renderLists(){
 ...
 for(const list of orderedLists.filter(l=>l.block!=='AIRPORT'&&listSection(l.block)===active)){
  ...
  for(const [i,item] of list.items.entries()){
   ...
   if(list.block==='food'&&!list.parentKey){const poster=document.createElement('button');poster.type='button';poster.className='field-choice';poster.textContent='대표 PHOTO · 언어별 포스터 관리';poster.dataset.posterItem=item.key;poster.onclick=()=>selectField(item.key+'.posters.ko');box.append(poster);}
