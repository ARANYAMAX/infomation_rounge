...
function render(preserveEditor=false){
 ...
 for(const field of catalog.filter(f=>f.id===selectedId)){
  ...
  if(field.type==='image'){
   ...
   if(field.path.at(-2)==='posters'){const language=document.createElement('select');language.setAttribute('aria-label','포스터 언어');for(const [l,n]of Object.entries({ko:'한국어',...languages})){const o=document.createElement('option');o.value=l;o.textContent=n;language.append(o);}language.value=field.path.at(-1);language.onchange=()=>selectField(field.block+':'+[...field.path.slice(0,-1),language.value].join('.'));card.append(language);const help=document.createElement('p');help.className='muted';help.textContent='이미지가 하나만 있다면 한국어 이미지만 등록해주세요. 해당 언어 이미지가 없으면 한국어 이미지가 표시됩니다.';card.append(help);}
   if(field.block==='MANUALS'&&(field.path.at(-1)==='image'||field.path.at(-2)==='imageLocales')){const language=document.createElement('select');language.setAttribute('aria-label','이미지 언어');const base=field.path.at(-1)==='image'?field.path.slice(0,-1):field.path.slice(0,-2);for(const [l,n]of Object.entries({ko:'한국어',...languages})){const o=document.createElement('option');o.value=l;o.textContent=n;language.append(o);}language.value=field.path.at(-1)==='image'?'ko':field.path.at(-1);language.onchange=()=>selectField('MANUALS:'+base.concat(language.value==='ko'?['image']:['imageLocales',language.value]).join('.'));const help=document.createElement('p');help.textContent='이미지가 하나만 있다면 한국어 이미지만 등록해주세요. 해당 언어 이미지가 없으면 한국어 이미지가 표시됩니다.';card.append(language,help);}
...
function renderLists(){
 ...
 const relevant=list=>!!list.parentKey&&(owner===list.parentKey||owner?.startsWith(list.parentKey+'.'));
 const orderedLists=[...lists].sort((a,b)=>Number(relevant(b))-Number(relevant(a))||(active==='amenities'?amenityOrder(a)-amenityOrder(b):0));
 for(const list of orderedLists.filter(l=>l.block!=='AIRPORT'&&listSection(l.block)===active)){
  const box=document.createElement('details');box.className='managed-list';box.dataset.listKey=list.key;box.open=relevant(list)|| (open.get(list.key)??!list.parentKey);const heading=document.createElement('summary');heading.textContent=(list.parentKey?(itemTitle(lists.flatMap(l=>l.items).find(i=>i.key===list.parentKey))+' · '):'')+(active==='amenities'?({'AMENITIES:hanok':'한옥만의 공간','AMENITIES:ess':'필수 시설','AMENITIES:kit':'주방·안전','aranya-essentials-data:places':'생활 편의(편의점·약국)'}[list.key]||list.label):(list.block==='TRAVEL_TIPS'&&list.path.at(-1)==='steps'?'세부 안내':list.label))+' · '+list.items.length+'개';box.append(heading);
  for(const [i,item] of list.items.entries()){
   ...
   if(['food','ARANYA_CURATED_PLACES'].includes(list.block)&&!list.parentKey){const poster=document.createElement('button');poster.type='button';poster.className='field-choice';poster.textContent='대표 PHOTO · 언어별 포스터 관리';poster.dataset.posterItem=item.key;poster.onclick=()=>selectField(item.key+'.posters.ko');box.append(poster);}
   if(owner===item.key||owner?.startsWith(item.key+'.')){const fields=document.createElement('div');fields.className='item-fields';const hidePhotos=['food','ARANYA_CURATED_PLACES'].includes(list.block);for(const id of item.fieldIds){const f=catalog.find(f=>f.id===id);if(!visibleField(f)||hidePhotos&&f.type==='image'&&f.path.at(-2)!=='posters'||f.path.at(-2)==='posters'&&f.path.at(-1)!=='ko')continue;const b=document.createElement('button');b.textContent=f.path.at(-2)==='posters'?(f.block==='food'?'대표 PHOTO · 언어별 포스터 관리':'언어별 포스터 관리'):f.type==='image'?'사진':fieldName(f);b.dataset.fieldId=id;b.onclick=()=>selectField(id);fields.append(b);}if(!hidePhotos&&photoGroups.some(p=>p.key===item.key))fields.append(actionButton('사진 추가',{type:'photoAdd',key:item.key}));box.append(fields);}
  }
  const add=document.createElement('div');add.className='managed-add';const b=document.createElement('button');b.type='button';b.textContent=list.path.at(-1)==='stops'?'+ 추천 가게 추가':'+ 항목 추가';b.onclick=()=>addListItem(list);add.append(b);box.append(add);root.append(box);
 }
