...
function visibleField(field){return field.type!=='url'&&field.path.at(-1)!=='naverSearch';}
...
function render(preserveEditor=false){
 ...
 for(const field of available){const choice=document.createElement('button');choice.type='button';choice.textContent=fieldName(field);choice.className='field-choice';choice.dataset.fieldId=field.id;choice.onclick=()=>selectField(field.id);$('fieldChoices').append(choice);}
 ...
 for(const field of catalog.filter(f=>f.id===selectedId)){
  ...
  }else if(field.block==='AMENITIES'&&field.path.at(-1)==='icon'){
...
$('guideFrame').onload=()=>{
 ...
 doc.addEventListener('click',event=>{if(busy&&!browseMode){event.preventDefault();event.stopImmediatePropagation();return;}const target=event.target.closest('a,button');if(target?.matches('a')){event.preventDefault();event.stopImmediatePropagation();if(!browseMode&&draft[target.dataset.cmsField])selectField(target.dataset.cmsField);return;}if(target?.matches('[data-page],#homeBrand')){guideNavigation=target.id==='homeBrand'?'home':target.dataset.page;if(!guideNavigating)syncEditorSection(guideNavigation);}if(browseMode||target?.matches('[data-page],#homeBrand,summary,[data-guide-photo],.guide-lightbox button'))return;let el=event.target;while(el&&!guideMatches.has(el))el=el.parentElement;if(!el)return;event.preventDefault();event.stopImmediatePropagation();const ids=guideMatches.get(el);if(ids.length===1){const f=catalog.find(f=>f.id===ids[0]);if(f?.block==='ARANYA_CURATED_PLACES'&&f.path.at(-1)==='group')toggleTravelGroup(f.id);else selectField(ids[0]);}else{$('fieldChoices').replaceChildren();for(const id of ids){const f=catalog.find(f=>f.id===id),b=document.createElement('button');b.className='field-choice';b.textContent=sections[group(f)]+' → '+fieldName(f);b.onclick=()=>selectField(id);$('fieldChoices').append(b);}$('fieldBrowser').open=true;$('editorPanel').classList.add('is-open');message('같은 문구가 여러 곳에 있습니다. 오른쪽에서 수정할 위치를 선택해주세요.');}},true);
...
function renderLists(){
 ...
 for(const list of orderedLists.filter(l=>listSection(l.block)===active&&(!l.parentKey||owner&&(owner===l.parentKey||owner.startsWith(l.parentKey+'.'))))){
  const box=document.createElement('details');box.className='managed-list';box.dataset.listKey=list.key;box.open=open.get(list.key)??!list.parentKey;const heading=document.createElement('summary');heading.textContent=(active==='amenities'?({'AMENITIES:hanok':'한옥만의 공간','AMENITIES:ess':'필수 시설','AMENITIES:kit':'주방·안전','aranya-essentials-data:places':'생활 편의(편의점·약국)'}[list.key]||list.label):list.label)+' · '+list.items.length+'개';box.append(heading);
  for(const [i,item] of list.items.entries()){
   ...
   if(owner===item.key||owner?.startsWith(item.key+'.')){const fields=document.createElement('div');fields.className='item-fields';const hidePhotos=['food','ARANYA_CURATED_PLACES'].includes(list.block);for(const id of item.fieldIds){const f=catalog.find(f=>f.id===id);if(!visibleField(f)||hidePhotos&&f.type==='image')continue;const b=document.createElement('button');b.textContent=f.type==='image'?'사진':fieldName(f);b.dataset.fieldId=id;b.onclick=()=>selectField(id);fields.append(b);}if(!hidePhotos&&photoGroups.some(p=>p.key===item.key))fields.append(actionButton('사진 추가',{type:'photoAdd',key:item.key}));box.append(fields);}
  }
 ...
 if(selected?.type==='image'){const photo=photoGroups.find(p=>p.fieldIds.includes(selectedId));if(photo)root.prepend(actionButton('이 사진 칸 삭제',{type:'photoRemove',key:photo.key,id:selected.path.at(-1)}));}
}
