...
function renderLists(){
 ...
 for(const list of lists.filter(l=>listSection(l.block)===active&&(!l.parentKey||owner&&(owner===l.parentKey||owner.startsWith(l.parentKey+'.'))))){
  const box=document.createElement('details');box.className='managed-list';box.dataset.listKey=list.key;box.open=open.get(list.key)??!list.parentKey;const heading=document.createElement('summary');heading.textContent=list.label+' · '+list.items.length+'개';box.append(heading);
