...
export function renderTravelExtras(copy,model,text,photos,escape,icons){
 ...
 const poster=item=>item.posters&&Object.values(item.posters).some(Boolean)?'<button type="button" class="place-photo-button" data-cms-field="'+escape(item._cmsKey+'.posters.ko')+'" data-localized-posters="'+escape(JSON.stringify(item.posters))+'" data-guide-photo="'+escape(item.posters.ko||item.posters.en||Object.values(item.posters).find(Boolean))+'">PHOTO</button>':'';
 const a=copy.AIRPORT;
 const airport='<div class="links" id="airportGuide"><button type="button" class="place-photo-button" data-cms-field="AIRPORT:posters.ko" data-localized-posters="'+escape(JSON.stringify(a.posters))+'" data-guide-photo="'+escape(a.posters.ko||a.posters.en||Object.values(a.posters).find(Boolean)||'')+'"><span aria-hidden="true">✈ </span>'+tag('span','AIRPORT',null,'button')+'</button></div>';
