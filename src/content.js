...
export function renderGuide(template,blocks,catalog,content){
 ...
 for(const field of catalog){const entry=content[field.id];if(!entry)continue;
  if(field.block==='amenityPhotos'){template=template.replace('__AMEN_PHOTO_'+field.path[0]+'__',entry.values.ko?'<button type="button" class="place-photo-button" data-guide-photo="'+entry.values.ko+'">PHOTO</button>':'');continue;}
