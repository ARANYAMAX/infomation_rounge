...
async function state(env){let row=await env.DB.prepare('SELECT * FROM content_state WHERE id=1').first();if(!row){await env.DB.prepare('INSERT OR IGNORE INTO content_state(id,revision,draft,published) VALUES(1,0,?,?)').bind(JSON.stringify(seed),JSON.stringify(seed)).run();row=await env.DB.prepare('SELECT * FROM content_state WHERE id=1').first();}return {revision:row.revision,draft:JSON.parse(row.draft),published:JSON.parse(row.published)};}
