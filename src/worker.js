...
function withCustom(value){return {...value,__custom:value.__custom||{food:[],around:[]}};}
async function state(env){let row=await env.DB.prepare('SELECT * FROM content_state WHERE id=1').first();if(!row){const initial=withCustom(seed);await env.DB.prepare('INSERT OR IGNORE INTO content_state(id,revision,draft,published) VALUES(1,0,?,?)').bind(JSON.stringify(initial),JSON.stringify(initial)).run();row=await env.DB.prepare('SELECT * FROM content_state WHERE id=1').first();}return {revision:row.revision,draft:withCustom(JSON.parse(row.draft)),published:withCustom(JSON.parse(row.published))};}
