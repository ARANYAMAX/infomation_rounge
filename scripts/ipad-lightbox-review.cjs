...
(async()=>{
 ...
 for(const [name,engine,options]of engines){const browser=await engine.launch({headless:true,...options});try{
  for(const size of [{width:1194,height:834},{width:834,height:1194},{width:390,height:844}]){
   ...
   await page.locator('.guide-lightbox button').click();await page.waitForFunction(()=>document.body.style.position!=='fixed');assert(Math.abs(await page.evaluate(()=>scrollY)-before)<3,'restores scroll');assert.deepEqual(errors,[]);console.log('PASS',name,size,'fit, close button, scroll lock, resize/rotation, scroll restore');await page.close();
  }
