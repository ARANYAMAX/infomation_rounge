...
html=html.replace('</head>',`<script src="/language-controls.js" defer></script><script>function aranyaEscape(v){return String(v||'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}function aranyaGuideImage(item){return item.imageLocales?.[typeof lang==='string'?lang:'ko']||item.image||'';}function aranyaPhotos(item){if(item.imageLocales)item={...item,image:aranyaGuideImage(item)};if(item._cmsKey?.startsWith('ARANYA_CURATED_PLACES:'))return aranyaDiningHero(item)+(item.photos||[]).filter(Boolean).map((url,i)=>'<button type="button" class="place-photo-button" data-guide-photo="'+aranyaEscape(url)+'">PHOTO '+(i+2)+'</button>').join('');const poster=item.posters&&Object.values(item.posters).some(Boolean)?'<button type="button" class="place-photo-button" data-cms-field="'+aranyaEscape(item._cmsKey+'.posters.ko')+'" data-localized-posters="'+aranyaEscape(JSON.stringify(item.posters))+'" data-guide-photo="'+aranyaEscape(item.posters.ko||item.posters.en||Object.values(item.posters).find(Boolean))+'">PHOTO</button>':'';return poster+[item.image,...(item.photos||[])].filter(Boolean).map((url,i)=>'<button type="button" class="place-photo-button" data-guide-photo="'+url+'">PHOTO '+(i+1)+'</button>').join('');}</script></head>`);
...
html=html.replace('</head>',`<script>function aranyaDiningHero(item){
 const posters=item.posters||{},code=typeof lang==='string'?lang:'ko';
 const posterLanguage=posters[code]?code:posters.ko?'ko':!item.image?(posters.en?'en':Object.keys(posters).find(l=>posters[l])):null;
...
replaceRequired('${m.image?`<div class="manual-media"><img src="${m.image}" alt="${aranyaEscape(arPick(m.title))}"></div>`:""}','');
html=html.replace(/\.manual-media(?: img)?\{[^}]*\}/g,'');
html=html.replace('</head>','<style>.manual-top{grid-template-columns:minmax(0,1fr)}.manual-body > .place-photo-button{margin:0 8px 14px 0}.manual-body > .quick-flow{gap:8px;margin:0 0 16px;align-items:center}.manual-body > .quick-flow .quick-step{padding:7px 11px;line-height:1.5}</style></head>');
