...
test('airport and tips fields use existing translations, stable lists and galleries',()=>{
 ...
 for(const f of model.catalog.filter(f=>['AIRPORT','TRAVEL_TIPS'].includes(f.block)&&f.type==='text')){for(const l of languages)assert.equal(typeof d[f.id].values[l],'string');const edited=structuredClone(d);edited[f.id].values.ko+=' 수정';assert(pending(validateDraft(edited,model.catalog,d),model.catalog).includes(f.id));}
...
test('airport stays inside location card and tips are translated between travel and dining',()=>{
 const html=renderGuide(template,blocks,catalog,h(seed));assert(html.includes('id="stationMaps"></div><details id="airportGuide"'));assert(!html.includes('data-page="airport"'));assert(!/<details id="airportGuide"[^>]*\bopen/.test(html));
